...
docker exec ${CONTAINER} bash -c "python3 /root/run_inference.py \
    ...
    --output_file /root/output_result.npy \
    --device cuda:0 \
    2>&1 | tee /root/inference.log"
```

**注意**：具体参数根据模型类型调整，output_file 格式可以是 `.npy`、`.json` 等。
...
docker exec ${CONTAINER} bash -c "
    ...
    test -s /root/output_result.npy && echo '✓ 输出文件非空' || echo '⚠ 无输出文件（检查是否指定了 --output_file）'
...
docker exec ${CONTAINER} bash -c "PATH=/opt/conda/bin:\$PATH python3 /Offline_inference_workspace/scripts/update_context.py \
    ...
    --set native_inference.output_path=/root/output_result.npy \
...
docker exec ${CONTAINER} bash -c "python3 /root/run_inference.py \
    --model_path ${MODEL_PATH} \
    --input_file /root/test_input.txt \
    --output_file /root/output_result_flaggems.npy \
    --device cuda:0 \
...
docker exec ${CONTAINER} bash -c "PATH=/opt/conda/bin:\$PATH python3 /Offline_inference_workspace/scripts/update_context.py \
    ...
    --set flaggems_inference.output_path=/root/output_result_flaggems.npy \
