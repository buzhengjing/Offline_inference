...
workflow:
  native_inference_ok: <来自 inference-verify>
  flaggems_inference_ok: <来自 inference-verify>
  all_done: <来自编排层>
```

## 写入容器内 /Offline_inference_workspace/shared/context.yaml
...
# 执行发布
python main.py --from-context /Offline_inference_workspace/shared/context.yaml

# 干运行（只验证配置，不实际执行）
python main.py --from-context /Offline_inference_workspace/shared/context.yaml --dry-run

# 只生成 README
python main.py --from-context /Offline_inference_workspace/shared/context.yaml --only-readme
```
...
- 容器内 `/Offline_inference_workspace/{results,traces,logs}/` 已通过 `docker cp` 同步到宿主机 `/data/Offline_inference_workspace/<model>/` 对应目录
...
if [ "$MOUNT_MODE" = "mounted" ] || [ "$MOUNT_MODE" = "symlink" ]; then
    cp /data/Offline_inference_workspace/<model>/shared/context.yaml /data/Offline_inference_workspace/<model>/config/context_snapshot.yaml
else
    docker cp <container>:/Offline_inference_workspace/shared/context.yaml /data/Offline_inference_workspace/<model>/config/context_snapshot.yaml
fi
python3 skills/flagos-release/tools/main.py --from-context /data/Offline_inference_workspace/<model>/config/context_snapshot.yaml
```
...
步骤8完成后，根据 `workspace.mount_mode` 决定同步策略（读取容器内 `/Offline_inference_workspace/.mount_mode`）：
...
HOST_BASE=/data/Offline_inference_workspace/<model>
MOUNT_MODE=$(docker exec ${CONTAINER} cat /Offline_inference_workspace/.mount_mode 2>/dev/null || echo "internal")
...
if [ "$MOUNT_MODE" = "mounted" ] || [ "$MOUNT_MODE" = "symlink" ]; then
    ...
    for dir in results traces logs; do
        docker cp ${CONTAINER}:/Offline_inference_workspace/${dir}/. ${HOST_BASE}/${dir}/
    done
    docker cp ${CONTAINER}:/Offline_inference_workspace/shared/context.yaml ${HOST_BASE}/config/context_snapshot.yaml
fi
```

**禁止回传到项目源码目录**。`docker cp` 目标必须是 `/data/Offline_inference_workspace/<model>/` 下的子目录。
...
```bash
docker exec $CONTAINER bash -c "PATH=/opt/conda/bin:\$PATH python3 /Offline_inference_workspace/scripts/generate_report.py \
  --output /Offline_inference_workspace/results/report.md"
docker exec $CONTAINER bash -c "PATH=/opt/conda/bin:\$PATH python3 /Offline_inference_workspace/scripts/generate_report.py \
  --json --output /Offline_inference_workspace/results/report.json"
```
...
python3 skills/flagos-release/tools/main.py \
    --from-context /data/Offline_inference_workspace/<model>/config/context_snapshot.yaml \
    --plugin-mode
```
