...
class PipelineConfig:
    ...
    container_name: str = ""
    host_workspace_base: str = ""  # /data/Offline_inference_workspace/<model>，由 context.yaml workspace.host_path 填充
...
def load_config_from_context(context_path: str) -> PipelineConfig:
    ...
    if (not config.publish.modelscope_token or not config.publish.huggingface_token
        ...
        try:
            result = subprocess.run(
                ["docker", "exec", config.container_name, "cat", "/Offline_inference_workspace/.env"],
    ...
    container_workspace = workspace.get('container_path', '/Offline_inference_workspace')
    ...
    # 宿主机工作目录（数据回传目标，应为 /data/Offline_inference_workspace/<model> 格式）
