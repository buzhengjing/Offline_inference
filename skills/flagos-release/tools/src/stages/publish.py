...
class PublishStage(BaseStage):
    ...
    def _sync_to_host(self):
        """将容器内 /Offline_inference_workspace 的产出同步到宿主机工作目录。
        ...
        # host_workspace_base 已包含完整路径（如 /data/Offline_inference_workspace/Qwen/Qwen2.5-0.5B-Instruct）
        ...
        for dir_name in sync_dirs:
            container_dir = f"/Offline_inference_workspace/{dir_name}"
        ...
        try:
            cp_result = subprocess.run(
                ["docker", "cp",
                 f"{container_name}:/Offline_inference_workspace/shared/context.yaml",
    ...
    def _ensure_harbor_login(self, harbor_path: str) -> bool:
        ...
        if user and password:
            ...
            try:
                ...
                login_env["HARBOR_PASSWORD"] = password
                result = subprocess.run(
                    f"printf '%s' \"$HARBOR_PASSWORD\" | docker login --username={user} --password-stdin https://{registry}/",
                    shell=True,
                    capture_output=True,
                    text=True,
                    timeout=60,
                ...
                if result.returncode == 0:
                    ...
                    self.steps.append(StepResult(
                        ...
                        error=result.stderr or "登录失败"
                    ))
                    return False
            except subprocess.TimeoutExpired:
                print(f"  x Harbor 登录超时")
                return False
...
"""
    ...
    def _update_repo_readme(self, repo_id: str, platform: str, readme_path: str) -> bool:
        ...
        for attempt in range(UPLOAD_MAX_RETRIES):
            ...
            try:
                result = subprocess.run(
                    docker_cmd, capture_output=True, text=True, timeout=UPLOAD_TIMEOUT
                )
                if result.returncode == 0:
